  ...
  return (
    <div className={`flex flex-col md:flex-row items-start gap-[80px] md:py-[100px] py-[80px] border-b border-gray-200 ${className}`}>
      ...
      <div className="flex-1">
        <h3 className="text-[35px] font-semibold mb-2">{stepTitle}</h3>
        ...
        <div className="flex gap-4">
          {buttons.map(({ label, link }, idx) => (
            <Link
              key={idx}
              to={link}
              className="inline-block bg-[#C3B3A3] text-white px-6 py-2 rounded-full text-sm hover:bg-[#c9b7a8] transition"
            >
              {label}
            </Link>
          ))}
      ...
      <div className="max-w-[1280px] mx-auto px-6 whitespace-pre-line">
        ...
        className=""
          ...
          stepDesc={`·추모에 쓰일 물품(간식, 장난감, 즐겨입던 옷)을 함께 챙겨오셔도 됩니다.\n·차량운전 및 방문이 힘드실 경우 대표번호(0000-0000)로 방문장례를 요청하실 있습니다.`}
           buttons={[
            { label: "온라인 예약 >", link: "/reservation" },
            { label: "선택형 예약 서비스 >", link: "tel:0000-0000" },
          ]}
        />
        ...
        <FuneralProcedure
          ...
          buttons={[
            { label: "운구서비스 >", link: "/reservation" },
            { label: "비동행 장례서비스 >", link: "tel:0000-0000" },
          ]}
        />
        ...
        />
         <FuneralProcedure
          ...
          stepDesc=""
          buttons={[
            { label: "메모리얼 스톤 >", link: "/reservation" },
